...
Один человек **хостит** на своём компе (нужен проект + БД). Второй подключается **только браузером** по Wi‑Fi/LAN.
...
В терминале появится строка **`LAN (мультиплеер): http://192.168.x.x:8765/`** — её открывает напарник.
...
**Важно:** хост должен держать app запущенным; экспорт в бот делает любой, кто открыт к хосту (файлы пишутся на комп хоста). Пароля нет — только доверенная сеть.
...
**Ссылка не открывается у друга**

1. Отправляй **Wi‑Fi IP** (`192.168.x.x`), не `172.31.x.x` — последний часто VPN и с другого компа не работает.
2. Оба в **одной Wi‑Fi** (не мобильный интернет у друга).
3. На Mac: *Системные настройки → Сеть → Firewall* — разреши входящие для **Python** (или временно выключи firewall для проверки).
4. Проверка у себя: открой `http://192.168.1.124:8765/` (свой IP из `ifconfig en0`).
