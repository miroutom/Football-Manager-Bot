...
def _nation_to_flagcdn_code(raw: str | None) -> str | None:
    ...
    from utils.player_nation import nation_to_flagcdn_code

    return nation_to_flagcdn_code(raw)
...
# Скамейка в сайдбаре: первые N игроков вне основы — «скамейка», остальные — «резерв»
# (типично до 8 строк резерва при 11+7+8 заявке).
SUBSTITUTES_COUNT = 7
