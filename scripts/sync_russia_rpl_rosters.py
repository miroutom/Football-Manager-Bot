#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Миграция ``status`` + синк заявок РПЛ в ``league_new.db`` / ``champions_league_new.db`` (участники ЛЧ) + ``common.db``."""
...
from utils.squad_roster_sync import run_russia_rpl_sync
...
def main() -> None:
    ...
    for tour, teams in stats.items():
        ...
        for team, s in teams.items():
            print(team, s)
    if "cl" in stats:
        skipped = [t for t in RUSSIA_RPL_SQUADS if t not in stats["cl"]]
        if skipped:
            print(
