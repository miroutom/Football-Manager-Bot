#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Миграция ``status`` + синк заявок Ла Лиги в ``league_new.db`` / ``champions_league_new.db`` (участники ЛЧ) + ``common.db``."""
...
from utils.squad_roster_sync import run_spain_la_liga_sync
...
def main() -> None:
    ...
    for tour, teams in stats.items():
        ...
        for team, s in teams.items():
            print(team, s)
    if "cl" in stats:
        skipped = [t for t in SPAIN_LA_LIGA_SQUADS if t not in stats["cl"]]
        if skipped:
            print(
