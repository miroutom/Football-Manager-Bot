#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Миграция ``status`` + синк заявок Бундеслиги в ``league_new.db`` / ``champions_league_new.db`` (только участники ЛЧ) + ``common.db``."""
...
from utils.squad_roster_sync import run_bundesliga_sync
...
def main() -> None:
    ...
    for tour, teams in stats.items():
        ...
        for team, s in teams.items():
            print(team, s)
    if "cl" in stats:
        skipped = [t for t in GERMANY_BUNDESLIGA_SQUADS if t not in stats["cl"]]
        if skipped:
            print(
